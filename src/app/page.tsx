...
export default function HomePage() {
  ...
  // SEO FIX 1: Initialize state with DEFAULT_WEIGHTS immediately.
  // This ensures the HTML generated by the server contains the full table.
  const [rankedSchools, setRankedSchools] = useState<any[]>(() => calculateRanking(DEFAULT_WEIGHTS));
  ...
  useEffect(() => {
    // SEO FIX 2: Remove the "if aggregates empty return" check.
    // Instead, fallback to DEFAULT_WEIGHTS if aggregates aren't ready.
    // This allows interactions (like 'isInState') to work even if live data fails or is loading.
    
    const activeAgg = aggregates.find(a => a.role === selectedRole) || aggregates.find(a => a.role === 'overall');
    
    // Fallback to DEFAULT_WEIGHTS if no live data yet
  ...
  const currentWeights = useMemo(() => {
    // SEO FIX 3: Ensure this never returns undefined/empty object if aggregates are loading
    const activeAgg = aggregates.find(a => a.role === selectedRole);
    return activeAgg?.weights || DEFAULT_WEIGHTS;
    ...
    <div className="min-h-screen pb-12 flex flex-col"> 
      ...
      <div className="max-w-[1600px] mx-auto px-4 mt-6 w-full">
        ...
        <div className="flex-1 min-h-0">
          {/* SEO FIX 4: Removed the loading check. The table always renders with at least default data. */}
          <RankingsTable 
